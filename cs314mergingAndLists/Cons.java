...
//	x = removeDupes(x);
//	y = removeDupes(y);
	while (first(rest(x)) != null && ((Comparable) first(x)).compareTo(first(rest(x))) == 0)
		x = rest(x);
	while (first(rest(y)) != null && ((Comparable) first(y)).compareTo(first(rest(y))) == 0)
		y = rest(y);
...
public static Cons mergediff (Cons x, Cons y) {
	...
	x = removeDupes(x);
	y = removeDupes(y);
	if (((Comparable) first(x)).compareTo(first(y)) < 0) return cons(first(x), mergediff(rest(x), y));
	else if (((Comparable) first(x)).compareTo(first(y)) > 0) return cons(first(y), mergediff(x, rest(y)));
	else return mergediff(rest(x), rest(y));
}

// Remove duplicate entries from the beginning of a list.
public static Cons removeDupes (Cons lst) {
	Cons tmp = lst;
	while (rest(tmp) != null) {
		while (first(rest(tmp)) != null && ((Comparable) first(tmp)).compareTo(first(rest(tmp))) == 0)
			tmp = rest(tmp);
		lst = tmp;	
	}
	return lst;
...
public static boolean markup(Cons text) {
    ...
    public static void main( String[] args )
      { 
        ...
        Cons list1 = list("a", "a", "b", "b", "c", "d");
        Cons list2 = list("f", "g", "h", "h", "k");
        ...
        System.out.println("list1 = " + Cons.toString(list1));
        System.out.println("list2 = " + Cons.toString(list2));
        System.out.println("list1 = " + Cons.toString(removeDupes(list1)));
        System.out.println("list2 = " + Cons.toString(removeDupes(list2)));
        System.out.println("union = " + Cons.toString(union(list1, list2)));
