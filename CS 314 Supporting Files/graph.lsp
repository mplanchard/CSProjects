; graph.lsp             Gordon S. Novak Jr.         ; 20 Apr 09; 29 Nov 12
...
(defun dijkstra (s)
  (let (fringe nodes v newcost)
    (dolist (v (vertices *txcities*))
      (setf (visited v) nil)
      (setf (parent v) nil)
      (setf (cost v) 999999) )
    (setf (cost s) 0)
    (setq nodes 0)
    (setq fringe (priorityqueue))
    (heap-add fringe s)
    (while (not (isempty fringe))
      (setq v (heap-remove fringe))
      (incf nodes)
      (if (not (visited v))
          (progn (setf (visited v) t)
                 (dolist (e (edges v))
                   (setq newcost (+ (cost v) (edge-cost e)))
                   (if (< newcost (cost (edge-target e)))
                       (progn (setf (cost (edge-target e)) newcost)
                              (setf (parent (edge-target e)) v)
                              (heap-add fringe (edge-target e)) ) ) ) ) ) )
    nodes))
