...
(redefpatterns 'javapats '(
       ...
       ( (cons ?x ?y) ("" cons "(" ?x "," " " ?y ")"))
       ( (zdefun ?fun ?args ?code) ("public static " ?fun " " (arglist ?args)
...
; (testderiv '(deriv (+ x 3) x) )
(defun testderiv (e)
  (let (der)
    (setq der (trans e 'derivpats))
    (format t "deriv:  ~A~%" e)
    (format t "  der:  ~A~%" der)
...
; test java patterns:
; (testjava '(setq j 3) )
; (dolist (x javatests) (testjava x) )
(defun testjava (x)
  (let (xrestruct xtrans)
    (terpri)
    (format t "java:  ~A~%" x)
    (setq xrestruct (trans x 'javarestructpats))
    (format t "       ~A~%" xrestruct)
    (setq xtrans (trans xrestruct 'javapats))
    (format t "       ~A~%" xtrans)
    (cpr xtrans) ))


(setq *compile-time-constants* '((pi "Math.pi")))
